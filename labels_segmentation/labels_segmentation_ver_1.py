...
while True:
    ...
    # Tạo một bản sao của ảnh để vẽ các lớp phủ bán trong suốt
    
    for entry in polygons_data_list:
        poly_to_draw = np.array(entry["polygon"], dtype=np.int32)
        if len(poly_to_draw) > 0:
            # Tạo một mask tạm thời cho mỗi polygon
            poly_mask = np.zeros(img_new.shape[:2], dtype=np.uint8)
            cv2.fillPoly(poly_mask, [poly_to_draw], 255)

            # Lấy màu dựa trên nhãn
            colors = [(255,0,0), (0,255,0), (0,0,255), (255,255,0), (255,0,255), (0,255,255), (128,0,0), (0,128,0), (0,0,128), (128,128,0)]
            label_index = int(entry["label"]) if entry["label"].isdigit() and 0 <= int(entry["label"]) < len(colors) else -1
            color_to_fill = colors[label_index] if label_index != -1 else (100, 100, 100) # Màu xám cho 'none'

            # Chuyển đổi màu sang BGR
            b, g, r = color_to_fill

            # Áp dụng màu mờ lên ảnh gốc
            overlay = img_new.copy()
            cv2.addWeighted(overlay, 1, np.zeros_like(overlay), 0, 0, overlay)
            cv2.addWeighted(img_new, 1, cv2.bitwise_and(overlay, overlay, mask=poly_mask), 0.5, 0, img_new)
            cv2.fillPoly(img_new, [poly_to_draw], color_to_fill)
            
            # Tính toán và vẽ điểm trung tâm
            center = calculate_center(poly_to_draw)
            if center:
                cv2.circle(img_new, center, 5, (255, 255, 255), -1)
                # Hiển thị nhãn của polygon
                label_text = name_label[entry["label"]][0] if entry["label"].isdigit() else "none"
                cv2.putText(img_new, label_text, (center[0] + 10, center[1] - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2, cv2.LINE_AA)
    ...
    if k == ord('q') or cv2.getWindowProperty(name_img, cv2.WND_PROP_VISIBLE) < 1:
        ...
        output_file = os.path.join(forder_output, name_img.split(".")[0] + ".txt")
        save_polygons_to_txt(img_new, output_file, polygons_data_list)
        break
    elif k == ord('?'):

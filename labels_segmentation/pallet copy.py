...
if __name__ == '__main__':
    ...
    while True:
        ...
        img_1= cv2.imread(r"C:\Xu_ly_anh\camera_data\anh_1_330.jpg")
        img_2= cv2.imread(r"C:\Xu_ly_anh\camera_data\anh_2_330.jpg")
...
## pose 
        ...
        # if len(bbox) > 0 and keypoints_tensor.numel() > 0:
        #     coords = keypoints_tensor[0].cpu().numpy()
        #     points_right_corners = coords.astype(np.float32)
        #     cv2.polylines(img_1, [coords.astype(np.int32)], isClosed=True, color=(0, 255, 0), thickness=2)

        # r2 = results_2[0]
        # bbox2 = r2.boxes.xyxy  
        # keypoints_tensor_2 = r2.keypoints.xy
        
        # if len(bbox2) > 0 and keypoints_tensor_2.numel() > 0:
        #     coords2 = keypoints_tensor_2[0].cpu().numpy()
        #     points_left_corners = coords2.astype(np.float32)
        #     cv2.polylines(img_2, [coords2.astype(np.int32)], isClosed=True, color=(0, 255, 0), thickness=2)
# detect
        r = results_1[0]
        boxes = r.boxes  

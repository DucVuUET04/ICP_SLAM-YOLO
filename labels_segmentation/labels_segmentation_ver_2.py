...
while True:
    ...
    # Thay vì so sánh tọa độ chính xác, chúng ta sẽ tìm sự tương ứng không gian.
    ...
    for new_contour in contours:
        ...
        if new_center:
            for old_entry in polygons_data_list:
                # Chỉ xem xét các vùng cũ đã có nhãn
                if old_entry['label'] != 'none':
                    old_center = calculate_center(old_entry['polygon'])
                    if old_center:
                        # Kiểm tra xem tâm của vùng mới có nằm trong vùng cũ không.
                        # Đây là một chỉ báo mạnh mẽ rằng chúng là cùng một đối tượng.
        ...
        # Gán nhãn tìm được. Nếu không có kết quả khớp, nó sẽ vẫn là "none".
        assigned_label = best_match_label
        
        # Nếu không khớp với bất kỳ vùng cũ nào, hãy kiểm tra các điểm mẫu được đặt thủ công
    ...
    cv2.moveWindow('mask ' + name_img, 500, 500)

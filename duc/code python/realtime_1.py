...
def draw_robot_pose(occupancy_map, pose, map_center_px, resolution, axis_length=300):
    """
    Vẽ vị trí và hướng của robot (hệ trục tọa độ) lên bản đồ chiếm dụng.
    - Vị trí: một vòng tròn màu hồng.
    - Hướng: hai đường thẳng đại diện cho trục X (màu đỏ) và Y (màu xanh lá).
    """
    ...
    # Dùng ma trận xoay để chuyển hướng của các trục sang hệ tọa độ toàn cục
    x_axis_end_vec = rotation @ x_axis_vec
    y_axis_end_vec = rotation @ y_axis_vec

    # Tính toán điểm cuối của các trục trong tọa độ pixel
    # Điểm cuối = điểm đầu (pixel) + vector hướng (đã chuyển sang pixel)
        ...
        while scanner.is_initialized():
            ...
            if len(global_map.points) < 100:
                ...
                if rmse > 50:
                    ...
                    # Khi RMSE tốt, cập nhật pose và thêm điểm mới vào bản đồ toàn cục
                    global_pose = transformation_matrix
                    current_points_global = transform_points(current_points, global_pose[:3, :3], global_pose[:3, 3])

                    # Lọc điểm và thêm vào bản đồ
